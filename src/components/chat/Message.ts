...
export async function addMessage(
	plugin: BMOGPT,
	input: string,
	messageType: "userMessage" | "botMessage",
	...
	if (messageType === "userMessage") {
		...
		const botMessageToolBarDiv =
			targetBotMessage?.querySelector(".botMessageToolBar");
		...
		if (
			!messageObj.content.includes("commandBotMessage") &&
			!messageObj.content.includes("errorBotMessage")
		) {
			...
			const appendButton = displayAppendButton(
				plugin,
				settings,
				messageObj.content,
			);
...
export function addParagraphBreaks(messageBlock: {
	querySelectorAll: (arg0: string) => NodeListOf<HTMLElement>;
}) {
