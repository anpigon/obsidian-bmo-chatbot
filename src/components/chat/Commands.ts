...
export function createBotMessage(settings: BMOSettings): HTMLDivElement {
	...
	botMessage.style.backgroundColor = colorToHex(
		settings.appearance.botMessageBackgroundColor ||
			getComputedStyle(document.body)
				.getPropertyValue(DEFAULT_SETTINGS.appearance.botMessageBackgroundColor)
				.trim(),
	);
	...
	botNameSpan.textContent =
		settings.appearance.chatbotName || DEFAULT_SETTINGS.appearance.chatbotName;
...
export async function commandModel(
	...
	if (!input.split(" ")[1]) {
		...
		const modelListItems = settings.allModels
	...
	if (input.split(" ")[1] !== undefined) {
		...
		if (Object.entries(modelAliases).find(([key, val]) => key === inputModel)) {
			...
			Object.entries(modelAliases).find(([key, val]) => val === inputModel)
		) {
			settings.general.model =
				modelAliases[
					Object.keys(modelAliases).find(
						(key) => modelAliases[key] === inputModel,
					) || ""
				];
			const commandBotMessage = `Updated Model to <b>${settings.general.model}</b>`;
			const botMessageDiv = displayCommandBotMessage(
...
export async function commandProfile(
	input: string,
	settings: BMOSettings,
	plugin: BMOGPT,
) {
	...
	const files = plugin.app.vault
		.getFiles()
		.filter((file) =>
			file.path.startsWith(plugin.settings.profiles.profileFolderPath),
		);
	...
	if (
		...
		if (profileAliases[inputValue]) {
			...
			const currentProfile = plugin.app.vault.getAbstractFileByPath(
				profileFilePath,
			) as TFile;
			plugin.activateView();
			...
			const matchedProfile = Object.entries(profileAliases).find(
				([key, value]) => value.toLowerCase() === inputValue.toLowerCase(),
			);
			if (matchedProfile) {
...
export async function commandPrompt(
	...
	const files = plugin.app.vault
		.getFiles()
		.filter((file) =>
			file.path.startsWith(plugin.settings.prompts.promptFolderPath),
		);
	...
	if (input.startsWith("/prompt")) {
		...
		if (promptAliases[inputValue]) {
			...
			// If input matches a value in profileAliases (case-insensitive)
			const matchedProfile = Object.entries(promptAliases).find(
				([key, value]) => value.toLowerCase() === inputValue.toLowerCase(),
			);
			if (matchedProfile) {
...
export async function commandReference(
	input: string,
	settings: BMOSettings,
	plugin: BMOGPT,
) {
	...
	if (inputValue === "true" || inputValue === "on") {
		...
		commandBotMessage +=
			"<p><strong>Type `/ref on` or `/ref off` to turn on/off reference current note.</strong></p>";
	}
...
export async function commandTemperature(
	input: string,
	settings: BMOSettings,
	plugin: BMOGPT,
) {
...
export async function commandMaxTokens(
	input: string,
	settings: BMOSettings,
	plugin: BMOGPT,
) {
...
export async function commandAppend(plugin: BMOGPT, settings: BMOSettings) {
	...
	if (activeFile?.extension === "md") {
		...
		const chatbotNameText =
			chatbotNames.length > 0 && chatbotNames[0].textContent
				? chatbotNames[0].textContent.toUpperCase()
				: "ASSISTANT";

		// Check and read the JSON file
		if (
			await this.app.vault.adapter.exists(filenameMessageHistoryJSON(plugin))
		) {
			try {
				const jsonContent = await this.app.vault.adapter.read(
				...
				markdownContent += messages
					.filter(
						(
							...
							if (
								messageHistory.content.startsWith("/") ||
								messageHistory.content.includes("errorBotMessage")
							) {
...
export async function commandSave(plugin: BMOGPT, settings: BMOSettings) {
	...
	try {
		...
		// Retrieve model name
		const modelNameElement = document.querySelector(
			"#modelName",
		) as HTMLHeadingElement;
		let modelName = "Unknown"; // Default model name
		if (modelNameElement?.textContent) {
			modelName = modelNameElement.textContent
				.replace("Model: ", "")
				.toLowerCase();
		}

		const templateFile = allFiles.find(
			(file) =>
				file.path.toLowerCase() ===
				settings.chatHistory.templateFilePath.toLowerCase(),
		);
...
---\n`;
		...
		// Check and read the JSON file
		if (
			await plugin.app.vault.adapter.exists(filenameMessageHistoryJSON(plugin))
		) {
			try {
				const jsonContent = await plugin.app.vault.adapter.read(
					filenameMessageHistoryJSON(plugin),
				);
				const messages = JSON.parse(jsonContent);
		...
		if (settings.chatHistory.allowRenameNoteTitle) {
			...
			while (!uniqueNameFound) {
				modelRenameTitle = await fetchModelRenameTitle(
					settings,
					markdownContent,
				);
