...
export async function fetchModelRenameTitle(
	settings: BMOSettings,
	referenceCurrentNoteContent: string,
) {
	const clearYamlContent = referenceCurrentNoteContent
		.replace(/---[\s\S]+?---/, "")
		.trim();
	...
	try {
		if (
			...
			throw new Error(
				"Invalid model selected for renaming note title. Please check your settings.",
			);
		}
	} catch (error) {
		console.log("ERROR");
