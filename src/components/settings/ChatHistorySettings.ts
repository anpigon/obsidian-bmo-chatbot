...
export function addChatHistorySettings(
	...
	new Setting(settingsContainer)
		...
		.addText((text) =>
			text
				...
				.setValue(
					plugin.settings.chatHistory.chatHistoryPath ||
						DEFAULT_SETTINGS.chatHistory.chatHistoryPath,
				)
				.onChange(async (value) => {
					...
					if (value) {
						let folderPath =
							plugin.settings.chatHistory.chatHistoryPath.trim() ||
							DEFAULT_SETTINGS.chatHistory.chatHistoryPath;
	...
	new Setting(settingsContainer)
		...
		.addText((text) =>
			text
				...
				.setValue(
					plugin.settings.chatHistory.templateFilePath ||
						DEFAULT_SETTINGS.chatHistory.templateFilePath,
				)
				.onChange(async (value) => {
					plugin.settings.chatHistory.templateFilePath = value
						? value
						: DEFAULT_SETTINGS.chatHistory.templateFilePath;
					if (value) {
						...
						const fileExists = allFiles.some(
							(file) =>
								...
								plugin.settings.chatHistory.templateFilePath.toLowerCase(),
						);
	...
	new Setting(settingsContainer)
		...
		.addToggle((toggle) =>
			toggle
				.setValue(plugin.settings.chatHistory.allowRenameNoteTitle)
				.onChange((value) => {
