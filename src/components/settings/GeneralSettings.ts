...
export async function addGeneralSettings(
	...
	new Setting(settingsContainer)
		...
		.setDesc(
			descLink(
				"The maximum number of tokens, or words, that the model is allowed to generate in its output.",
	...
	new Setting(settingsContainer)
		...
		.setDesc(
			"Allow chatbot to reference current active note during conversation.",
		)
		.addToggle((toggle) =>
			toggle
				.setValue(plugin.settings.general.allowReferenceCurrentNote)
				.onChange((value) => {
...
async function populateDropdownWithModels(
	plugin: BMOGPT,
	...
	for (const { type, condition } of modelSources) {
		if (condition) {
			try {
				...
				new Notice(
					`${type.charAt(0).toUpperCase() + type.slice(1)} connection error.`,
				);
			}
		}
	}
