...
export function addMistralConnectionSettings(
			...
			text
				...
				.setValue(
					plugin.settings.APIConnections.mistral.APIKey
						? `${plugin.settings.APIConnections.mistral.APIKey.slice(
								0,
								6,
						  )}-...${plugin.settings.APIConnections.mistral.APIKey.slice(-4)}`
						: "",
				)
	...
	new Setting(settingsContainer)
		...
		.addToggle((toggle) =>
			toggle
				.setValue(plugin.settings.APIConnections.mistral.allowStream)
				.onChange((value) => {
