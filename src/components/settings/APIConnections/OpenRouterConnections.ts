...
export function addOpenRouterConnectionSettings(
			...
			text
				...
				.setValue(
					plugin.settings.APIConnections.openRouter.APIKey
						? `${plugin.settings.APIConnections.openRouter.APIKey.slice(
								0,
								6,
						  )}-...${plugin.settings.APIConnections.openRouter.APIKey.slice(
								-4,
	...
	new Setting(settingsContainer)
		...
		.addToggle((toggle) =>
			toggle
				.setValue(plugin.settings.APIConnections.openRouter.allowStream)
				.onChange((value) => {
