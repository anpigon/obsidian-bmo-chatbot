...
export function addOpenAIConnectionSettings(
			...
			text
				...
				.setValue(
					plugin.settings.APIConnections.openAI.APIKey
						? `${plugin.settings.APIConnections.openAI.APIKey.slice(
								0,
								6,
						  )}-...${plugin.settings.APIConnections.openAI.APIKey.slice(-4)}`
						: "",
				)
	...
	new Setting(settingsContainer)
		...
		.addButton((button) =>
			button
				...
				.onClick(async () => {
					...
					plugin.settings.APIConnections.openAI.openAIBaseUrl =
		...
		.addText((text) =>
			text
				...
				.onChange(async (value) => {
					plugin.settings.APIConnections.openAI.openAIBaseUrl = value
						? value
						: DEFAULT_SETTINGS.APIConnections.openAI.openAIBaseUrl;
					await plugin.saveSettings();
				})
	...
	new Setting(settingsContainer)
		...
		.addToggle((toggle) =>
			toggle
				.setValue(plugin.settings.APIConnections.openAI.allowStream)
				.onChange(async (value) => {
