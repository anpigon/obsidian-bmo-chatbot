...
export async function fetchXionicResponse(plugin: BMOGPT, settings: BMOSettings, index: number) {
		...
		const response = await requestUrl({
			...
			body: JSON.stringify({
				...
				messages: [
					{
						role: 'system',
						content: settings.general.system_role + prompt + referenceCurrentNoteContent || 'You are a helpful assistant.',
					},
					...messageHistoryAtIndex,
				],
