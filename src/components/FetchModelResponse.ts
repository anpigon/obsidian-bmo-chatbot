...
} from "./chat/BotMessage";
import {
	getActiveFileContent,
	getCurrentNoteContent,
} from "./editor/ReferenceCurrentNote";
...
export async function fetchXionicResponse(
	plugin: BMOGPT,
	...
	const messageHistoryAtIndex = removeConsecutiveUserRoles(
	...
	messageContainerEl?.insertBefore(
		botMessageDiv,
		messageContainerElDivs[index + 1],
	);
			...
			messages: [
				{
					role: "system",
					content:
						settings.general.system_role + prompt + referenceCurrentNoteContent,
				},
	...
	try {
		const response = await fetch(url, {
			...
			body: JSON.stringify({
				...
				stream: true,
				keep_alive: parseInt(
					settings.OllamaConnection.ollamaParameters.keep_alive,
				),
...
export async function fetchRESTAPIURLResponse(
	plugin: BMOGPT,
	settings: BMOSettings,
	index: number,
) {
				...
				messages: [
					{
						...
						content:
							settings.general.system_role +
								prompt +
								referenceCurrentNoteContent || "You are a helpful assistant.",
					},
					...messageHistoryAtIndex,
				],
...
export async function fetchRESTAPIURLResponseStream(
	plugin: BMOGPT,
	settings: BMOSettings,
	index: number,
) {
...
export async function fetchGoogleGeminiResponse(
	...
	const convertMessageHistory = (
		...
		const convertedMessageHistory = modifiedMessageHistory.map(
			({ role, content }) => ({
...
export async function fetchMistralResponse(
	plugin: BMOGPT,
	settings: BMOSettings,
	index: number,
) {
...
export async function fetchMistralResponseStream(
	plugin: BMOGPT,
	settings: BMOSettings,
	index: number,
) {
...
export async function fetchOpenAIAPIResponse(
	plugin: BMOGPT,
	settings: BMOSettings,
	index: number,
) {
...
export async function fetchOpenAIAPIResponseStream(
	plugin: BMOGPT,
	settings: BMOSettings,
	index: number,
) {
...
// Fetch response from openai-based rest api url (stream)
export async function fetchOpenRouterResponseStream(
	plugin: BMOGPT,
	settings: BMOSettings,
	index: number,
) {
...
function ollamaParametersOptions(settings: BMOSettings) {
	return {
		...
		mirostat_eta: parseFloat(
			settings.OllamaConnection.ollamaParameters.mirostat_eta,
		),
		mirostat_tau: parseFloat(
			settings.OllamaConnection.ollamaParameters.mirostat_tau,
		),
		...
		repeat_last_n: parseInt(
			settings.OllamaConnection.ollamaParameters.repeat_last_n,
		),
		repeat_penalty: parseFloat(
			settings.OllamaConnection.ollamaParameters.repeat_penalty,
		),
		temperature: parseInt(settings.general.temperature),
...
function filterMessageHistory(
	...
	messageHistory.forEach((message, index, array) => {
		...
		// Check for assistant message with displayErrorBotMessage
			...
			message.content.includes("errorBotMessage")
		) {
			skipIndexes.add(index); // Skip this message
			if (index > 0) {
...
function removeConsecutiveUserRoles(
	messageHistory: { role: string; content: string }[],
) {
	const result = [];
