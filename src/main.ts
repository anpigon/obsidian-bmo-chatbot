...
export default class BMOGPT extends Plugin {
	...
	async onload() {
		...
		const folderPath =
			this.settings.profiles.profileFolderPath ||
			DEFAULT_SETTINGS.profiles.profileFolderPath;

		const defaultFilePath = `${folderPath}/${DEFAULT_SETTINGS.profiles.profile}`;
		const defaultProfile = this.app.vault.getAbstractFileByPath(
			defaultFilePath,
		) as TFile;
		...
		this.registerEvent(
			this.app.vault.on("delete", async (file: TFile) => {
				if (file instanceof TFile && file.path.startsWith(folderPath)) {
					const filenameMessageHistory = `./.obsidian/plugins/bmo-chatbot/data/messageHistory_${file.name.replace(
						".md",
						".json",
					)}`;
					...
					if (file.path === defaultFilePath) {
						...
						if (this.settings.profiles.profile === file.name) {
							this.settings.profiles.profile =
								DEFAULT_SETTINGS.profiles.profile;
							const fileContent = (await this.app.vault.read(defaultProfile))
								.replace(/^---\s*[\s\S]*?---/, "")
								.trim();
		...
		this.registerEvent(
			this.app.vault.on("rename", async (file: TFile, oldPath: string) => {
				try {
					...
					if (file instanceof TFile && file.path.startsWith(folderPath)) {
						const filenameMessageHistoryPath =
							"./.obsidian/plugins/bmo-chatbot/data/";
						const oldProfileMessageHistory = `messageHistory_${oldPath
							.replace(`${folderPath}/`, "")
							.replace(".md", ".json")}`;
						...
						await this.app.vault.adapter
							.rename(
								...
								`${filenameMessageHistoryPath}messageHistory_${file.name.replace(
									".md",
									".json",
								)}`,
							)
					...
					if (
						error.message.includes("ENOENT: no such file or directory, unlink")
					) {
	...
	async saveSettings() {
		...
		const currentProfile = this.app.vault.getAbstractFileByPath(
			currentProfileFile,
		) as TFile;
...
export async function defaultFrontMatter(plugin: BMOGPT, file: TFile) {
	...
	const setDefaultFrontMatter = async (frontmatter: any) => {
		...
		frontmatter.reference_current_note =
			DEFAULT_SETTINGS.general.allowReferenceCurrentNote;
		...
		frontmatter.chatbot_container_background_color =
			DEFAULT_SETTINGS.appearance.chatbotContainerBackgroundColor.replace(
				/^#/,
				"",
			);
		frontmatter.message_container_background_color =
			DEFAULT_SETTINGS.appearance.messageContainerBackgroundColor.replace(
				/^#/,
				"",
			);
		frontmatter.user_message_font_color =
			DEFAULT_SETTINGS.appearance.userMessageFontColor.replace(/^#/, "");
		frontmatter.user_message_background_color =
			DEFAULT_SETTINGS.appearance.userMessageBackgroundColor.replace(/^#/, "");
		frontmatter.bot_message_font_color =
			DEFAULT_SETTINGS.appearance.botMessageFontColor.replace(/^#/, "");
		frontmatter.chatbot_message_background_color =
			DEFAULT_SETTINGS.appearance.botMessageBackgroundColor.replace(/^#/, "");
		frontmatter.chatbox_font_color =
			DEFAULT_SETTINGS.appearance.chatBoxFontColor.replace(/^#/, "");
		frontmatter.chatbox_background_color =
			DEFAULT_SETTINGS.appearance.chatBoxBackgroundColor.replace(/^#/, "");
		frontmatter.prompt_select_generate_system_role =
...
export async function updateSettingsFromFrontMatter(
	...
	const updateSettings = async (frontmatter: any) => {
		...
		plugin.settings.general.allowReferenceCurrentNote =
			frontmatter.reference_current_note;
...
export async function updateFrontMatter(plugin: BMOGPT, file: TFile) {
	...
	const modifyFrontMatter = async (frontmatter: any) => {
		...
		frontmatter.chatbot_container_background_color =
			plugin.settings.appearance.chatbotContainerBackgroundColor.replace(
				/^#/,
				"",
			);
		frontmatter.message_container_background_color =
			plugin.settings.appearance.messageContainerBackgroundColor.replace(
				/^#/,
				"",
			);
		frontmatter.user_message_font_color =
			plugin.settings.appearance.userMessageFontColor.replace(/^#/, "");
		frontmatter.user_message_background_color =
			plugin.settings.appearance.userMessageBackgroundColor.replace(/^#/, "");
		frontmatter.bot_message_font_color =
			plugin.settings.appearance.botMessageFontColor.replace(/^#/, "");
		frontmatter.chatbot_message_background_color =
			plugin.settings.appearance.botMessageBackgroundColor.replace(/^#/, "");
		frontmatter.chatbox_font_color =
			plugin.settings.appearance.chatBoxFontColor.replace(/^#/, "");
		frontmatter.chatbox_background_color =
			plugin.settings.appearance.chatBoxBackgroundColor.replace(/^#/, "");
		frontmatter.prompt_select_generate_system_role =
			plugin.settings.editor.prompt_select_generate_system_role;
		frontmatter.ollama_mirostat = parseFloat(
			plugin.settings.OllamaConnection.ollamaParameters.mirostat,
		...
		await plugin.app.fileManager.processFrontMatter(
			...
			(frontmatter: any) => {
				...
				if (frontmatter.user_name) {
					plugin.settings.appearance.userName = frontmatter.user_name.substring(
						0,
						30,
					);
				} else {
				...
				if (isValidHexColor(frontmatter.chatbot_container_background_color)) {
					plugin.settings.appearance.chatbotContainerBackgroundColor = `#${frontmatter.chatbot_container_background_color.substring(
						0,
						6,
					)}`;
					if (chatbotContainer) {
						chatbotContainer.style.backgroundColor =
							plugin.settings.appearance.chatbotContainerBackgroundColor;
					}
				} else {
					plugin.settings.appearance.chatbotContainerBackgroundColor =
						colorToHex(
					...
					if (chatbotContainer) {
						const defaultChatbotContainerBackgroundColor = getComputedStyle(
							document.body,
						)
							.getPropertyValue(
								DEFAULT_SETTINGS.appearance.chatbotContainerBackgroundColor,
							...
							.trim();
						chatbotContainer.style.backgroundColor =
							defaultChatbotContainerBackgroundColor;
					}
				}

				if (isValidHexColor(frontmatter.message_container_background_color)) {
					plugin.settings.appearance.messageContainerBackgroundColor = `#${frontmatter.message_container_background_color.substring(
						0,
						6,
					)}`;
					if (messageContainer) {
						messageContainer.style.backgroundColor =
							plugin.settings.appearance.messageContainerBackgroundColor;
					}
				} else {
					plugin.settings.appearance.messageContainerBackgroundColor =
						colorToHex(
					...
					if (messageContainer) {
						const defaultMessageContainerBackgroundColor = getComputedStyle(
							document.body,
						)
							.getPropertyValue(
								DEFAULT_SETTINGS.appearance.messageContainerBackgroundColor,
							...
							.trim();
						messageContainer.style.backgroundColor =
							defaultMessageContainerBackgroundColor;
					}
				}

				if (isValidHexColor(frontmatter.user_message_font_color)) {
					plugin.settings.appearance.userMessageFontColor = `#${frontmatter.user_message_font_color.substring(
						0,
						6,
					)}`;
					if (messageContainer) {
						const userMessages =
							messageContainer.querySelectorAll(".userMessage");
						userMessages.forEach((userMessage) => {
							const element = userMessage as HTMLElement;
							element.style.color =
								plugin.settings.appearance.userMessageFontColor;
						});
					}
				} else {
					plugin.settings.appearance.userMessageFontColor = colorToHex(
					...
					if (messageContainer) {
						...
						userMessages.forEach((userMessage) => {
							const element = userMessage as HTMLElement;
							const defaultUserMessageFontColor = getComputedStyle(
								document.body,
							)
								.getPropertyValue(
									DEFAULT_SETTINGS.appearance.userMessageFontColor,
				...
				if (isValidHexColor(frontmatter.user_message_background_color)) {
					plugin.settings.appearance.userMessageBackgroundColor = `#${frontmatter.user_message_background_color.substring(
						0,
						6,
					)}`;
					if (messageContainer) {
						...
						userMessages.forEach((userMessage) => {
							...
							element.style.backgroundColor =
								plugin.settings.appearance.userMessageBackgroundColor;
						});
					}
				} else {
					plugin.settings.appearance.userMessageBackgroundColor = colorToHex(
					...
					if (messageContainer) {
						...
						userMessages.forEach((userMessage) => {
							const element = userMessage as HTMLElement;
							const defaultUserMessageBackgroundColor = getComputedStyle(
								document.body,
							)
								.getPropertyValue(
									DEFAULT_SETTINGS.appearance.userMessageBackgroundColor,
				...
				if (isValidHexColor(frontmatter.bot_message_font_color)) {
					plugin.settings.appearance.botMessageFontColor = `#${frontmatter.bot_message_font_color.substring(
						0,
						6,
					)}`;
					if (messageContainer) {
						...
						botMessages.forEach((botMessage) => {
							...
							element.style.color =
								plugin.settings.appearance.botMessageFontColor;
						});
					}
				} else {
					plugin.settings.appearance.botMessageFontColor = colorToHex(
					...
					if (messageContainer) {
						...
						botMessages.forEach((botMessage) => {
							const element = botMessage as HTMLElement;
							const defaultBotMessageFontColor = getComputedStyle(document.body)
								.getPropertyValue(
									DEFAULT_SETTINGS.appearance.botMessageFontColor,
								)
								.trim();
				...
				if (isValidHexColor(frontmatter.chatbot_message_background_color)) {
					plugin.settings.appearance.botMessageBackgroundColor = `#${frontmatter.chatbot_message_background_color.substring(
						0,
						6,
					)}`;
					if (messageContainer) {
						...
						botMessages.forEach((botMessage) => {
							const element = botMessage as HTMLElement;
							element.style.backgroundColor =
								plugin.settings.appearance.botMessageBackgroundColor;
						});
					}
				} else {
					plugin.settings.appearance.botMessageBackgroundColor = colorToHex(
					...
					if (messageContainer) {
						...
						botMessages.forEach((botMessage) => {
							const element = botMessage as HTMLElement;
							const defaultBotMessageBackgroundColor = getComputedStyle(
								document.body,
							)
								.getPropertyValue(
									DEFAULT_SETTINGS.appearance.botMessageBackgroundColor,
				...
				if (isValidHexColor(frontmatter.chatbox_font_color)) {
					plugin.settings.appearance.chatBoxFontColor = `#${frontmatter.chatbox_font_color.substring(
						0,
						6,
					)}`;
					const textarea = document.querySelector(
					...
					plugin.settings.appearance.chatBoxFontColor = colorToHex(
					...
					const defaultChatBoxFontColor = getComputedStyle(document.body)
						.getPropertyValue(DEFAULT_SETTINGS.appearance.chatBoxFontColor)
						.trim();
				...
				if (isValidHexColor(frontmatter.chatbox_background_color)) {
					plugin.settings.appearance.chatBoxBackgroundColor = `#${frontmatter.chatbox_background_color.substring(
						0,
						6,
					)}`;
					...
					if (messageContainer) {
						...
						if (chatbox) {
							...
							element.style.backgroundColor =
								plugin.settings.appearance.chatBoxBackgroundColor;
				...
				if (isNaN(parseFloat(frontmatter.ollama_mirostat_eta))) {
					...
					plugin.settings.OllamaConnection.ollamaParameters.mirostat_eta =
						parseFloat(frontmatter.ollama_mirostat_eta).toFixed(2).toString();
					frontmatter.ollama_mirostat_eta = parseFloat(
						plugin.settings.OllamaConnection.ollamaParameters.mirostat_eta,
					);
				}
				...
				if (isNaN(parseFloat(frontmatter.ollama_mirostat_tau))) {
					...
					plugin.settings.OllamaConnection.ollamaParameters.mirostat_tau =
						parseFloat(frontmatter.ollama_mirostat_tau).toFixed(2).toString();
					frontmatter.ollama_mirostat_tau = parseFloat(
						plugin.settings.OllamaConnection.ollamaParameters.mirostat_tau,
					);
				}
				...
				if (isNaN(parseInt(frontmatter.ollama_num_ctx))) {
					...
					plugin.settings.OllamaConnection.ollamaParameters.num_ctx = parseInt(
						frontmatter.ollama_num_ctx,
					).toString();
					frontmatter.ollama_num_ctx = parseInt(
						plugin.settings.OllamaConnection.ollamaParameters.num_ctx,
				...
				if (isNaN(parseInt(frontmatter.ollama_num_gqa))) {
					...
					plugin.settings.OllamaConnection.ollamaParameters.num_gqa = parseInt(
						frontmatter.ollama_num_gqa,
					).toString();
					frontmatter.ollama_num_gqa = parseInt(
						plugin.settings.OllamaConnection.ollamaParameters.num_gqa,
				...
				if (isNaN(parseFloat(frontmatter.ollama_repeat_penalty))) {
					...
					plugin.settings.OllamaConnection.ollamaParameters.repeat_penalty =
						parseFloat(frontmatter.ollama_repeat_penalty).toFixed(2).toString();
					frontmatter.ollama_repeat_penalty = parseFloat(
						plugin.settings.OllamaConnection.ollamaParameters.repeat_penalty,
					);
				}
				...
				if (isNaN(parseInt(frontmatter.ollama_seed))) {
					...
					plugin.settings.OllamaConnection.ollamaParameters.seed = parseInt(
						frontmatter.ollama_seed,
					).toString();
					frontmatter.ollama_seed = parseInt(
						plugin.settings.OllamaConnection.ollamaParameters.seed,
				...
				if (isNaN(parseFloat(frontmatter.ollama_tfs_z))) {
					...
					plugin.settings.OllamaConnection.ollamaParameters.tfs_z = parseFloat(
						frontmatter.ollama_tfs_z,
					)
						.toFixed(2)
						.toString();
				...
				if (isNaN(parseInt(frontmatter.ollama_top_k))) {
					...
					plugin.settings.OllamaConnection.ollamaParameters.top_k = parseInt(
						frontmatter.ollama_top_k,
					).toString();
					frontmatter.ollama_top_k = parseInt(
						plugin.settings.OllamaConnection.ollamaParameters.top_k,
				...
				if (isNaN(parseInt(frontmatter.ollama_top_p))) {
					...
					plugin.settings.OllamaConnection.ollamaParameters.top_p = parseFloat(
						frontmatter.ollama_top_p,
					)
						.toFixed(2)
						.toString();
