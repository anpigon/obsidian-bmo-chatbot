...
export function filenameMessageHistoryJSON(plugin: BMOGPT) {
	...
	const currentProfileMessageHistory = `messageHistory_${plugin.settings.profiles.profile.replace(
		".md",
		".json",
	)}`;
...
export class BMOView extends ItemView {
	...
	async BMOchatbot() {
		...
		if (this.settings.general.model === "") {
			...
			if (this.settings.general.model === DEFAULT_MODEL) {
				await fetchXionicResponse(this.plugin, this.settings, index);
			}
			// Fetch OpenAI API
			else if (
