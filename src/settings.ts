...
export class BMOSettingTab extends PluginSettingTab {
	...
	async display(): Promise<void> {
		...
		const resetButton = containerEl.createEl("a", {
			...
			attr: {
				style:
					"display: block; text-align: center; margin: 1rem 0; font-size: 0.7rem; color: #ff6666;",
			},
		});
		...
		resetButton.addEventListener("click", async (event) => {
			...
			if (confirmReset) {
				...
				const profilePath = this.plugin.app.vault.getAbstractFileByPath(
					profilePathFile,
				) as TFile;

				const defaultProfilePathFile = `${DEFAULT_SETTINGS.profiles.profileFolderPath}/${DEFAULT_SETTINGS.profiles.profile}`;
				const defaultProfilePath = this.plugin.app.vault.getAbstractFileByPath(
					defaultProfilePathFile,
				) as TFile;
				...
				if (profilePath) {
					if (profilePath.path === defaultProfilePath.path) {
						...
						// @ts-ignore
						await this.plugin.app.plugins.disablePlugin(
							this.plugin.manifest.id,
						);
						// @ts-ignore
						await this.plugin.app.plugins.enablePlugin(this.plugin.manifest.id);
					} else {
						const filenameMessageHistory = `./.obsidian/plugins/bmo-chatbot/data/messageHistory_${defaultProfilePath.name.replace(
							".md",
							".json",
						)}`;
						this.app.vault.adapter.remove(filenameMessageHistory);
						this.plugin.app.vault.delete(profilePath);
						this.plugin.settings.profiles.profile =
							DEFAULT_SETTINGS.profiles.profile;
						await updateSettingsFromFrontMatter(
							this.plugin,
							defaultProfilePath,
						);
						await this.plugin.saveSettings();
